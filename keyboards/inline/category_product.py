...
async def categories_keyboard():
    ...
    categories = await db_prod.get_categories()
    # Har bir kategoriya uchun quyidagilarni bajaramiz:
    for category in categories:
        # Kategoriyaga tegishli mahsulotlar sonini topamiz
        print(category)
        number_of_items = await db_prod.count_products(category["category_code"])
...
async def subcategories_keyboard(category):
    ...
    subcategories = await db_prod.get_subcategories(category)
    for subcategory in subcategories:
        # Kategoriyada qancha mahsulot borligini tekshiramiz
        number_of_items = await db_prod.count_products(
...
async def items_keyboard(category, subcategory):
    ...
    items = await db_prod.get_products(category, subcategory)
    for item in items:
        # Tugma matnini yasaymiz
